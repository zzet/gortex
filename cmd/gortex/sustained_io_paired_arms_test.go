...
// verdict out.
//
// The sustained harness (sustained_io_integration_test.go) measures ONE arm
...
//
//   - sustainedIOFreezeBudgets takes ONLY baseline runs and refuses anything else, so a
//     budget can never be derived from a number the candidate produced;
//   - sustainedIOWriteFrozenBudgets refuses to overwrite an existing budgets.json and
...
// sustainedIORunArtifact is one arm-repetition's report.json, as (*sustainedIORun).finish writes
// it.
type sustainedIORunArtifact struct {
	Manifest             sustainedIOManifest      `json:"manifest"`
	Phases               []sustainedIOPhaseReport `json:"phases"`
	FailedPhases         []string                 `json:"failed_phases"`
	RetainedCensus       sustainedIOCensus        `json:"retained_census"`
...
// sustainedIOLoadArmRuns reads every `<arm>_rep<N>/report.json` under dir, in repetition
...
// a reader can see.
func sustainedIOLoadArmRuns(dir, arm string) ([]sustainedIORunArtifact, error) {
	...
	runs := make([]sustainedIORunArtifact, 0, len(found))
	for _, entry := range found {
		data, err := os.ReadFile(entry.path)
		if err != nil {
			return nil, fmt.Errorf("%s: %w", entry.name, err)
		}
		var run sustainedIORunArtifact
		...
		sustainedIOFillCheckpointSeries(&run)
		runs = append(runs, run)
	}
	return runs, nil
}

// sustainedIOFillCheckpointSeries makes sure every phase of a run carries the
...
// to them is fixed in sustainedIOCheckpointSeries — so deriving the series afterwards
// moves no goalpost: it reads a series that was already recorded, out of bytes
// that were already frozen.
func sustainedIOFillCheckpointSeries(run *sustainedIORunArtifact) {
	missing := false
	...
	samples, skipped, err := sustainedIOReadSamples(filepath.Join(run.Dir, "samples.ndjson"))
	if err != nil {
	...
	for i := range run.Phases {
		...
		for j := range report.Windows {
			...
			window.CheckpointBytes = series.Windows[sustainedIOWindowKey(report.Phase, window.Window)]
			window.LogicalWritesExcl = sustainedIOExcludeCheckpoint(window.LogicalWrites, window.CheckpointBytes)
		}
	}
}
...
// sustainedIOStat is one series across repetitions. Unavailable names the repetitions
// that carried no value; N counts only the ones that did.
type sustainedIOStat struct {
	N           int      `json:"n"`
...
func (s sustainedIOStat) OK() bool { return s.N > 0 }

// sustainedIOStatistic reduces one series. A nil reading is named, never zeroed: three
...
func sustainedIOStatistic(labels []string, values []*uint64) sustainedIOStat {
	stat := sustainedIOStat{}
...
func sustainedIOUint64(value uint64) *uint64 { return &value }

// ---------------------------------------------------------------- budgets ---

// sustainedIOPhaseBudget is one phase's frozen baseline, plus the ceiling the candidate
// is judged against.
//
...
type sustainedIOPhaseBudget struct {
	...
	LogicalWritesExcl sustainedIOStat `json:"ri_logical_writes_excl_checkpoint,omitzero"`
	CheckpointBytes   sustainedIOStat `json:"wal_checkpoint_bytes,omitzero"`
	...
	LogicalWrites   sustainedIOStat `json:"ri_logical_writes"`
	DiskWritten     sustainedIOStat `json:"ri_diskio_byteswritten"`
	WALBytesAfter   sustainedIOStat `json:"wal_bytes_after"`
	WALResets       sustainedIOStat `json:"wal_resets"`
	StoreBytesAfter sustainedIOStat `json:"store_bytes_after"`
	GenerationsMax  sustainedIOStat `json:"generation_max_after"`
	GenerationSeq   sustainedIOStat `json:"generation_sequence_after"`
	...
	Windows []sustainedIOWindowBudget `json:"windows,omitempty"`
}

// sustainedIOWindowBudget is one sub-window's frozen baseline. It carries no ceiling:
...
type sustainedIOWindowBudget struct {
	Window            string          `json:"window"`
	Detail            string          `json:"detail,omitempty"`
	LogicalWrites     sustainedIOStat `json:"ri_logical_writes"`
	LogicalWritesExcl sustainedIOStat `json:"ri_logical_writes_excl_checkpoint"`
	CheckpointBytes   sustainedIOStat `json:"wal_checkpoint_bytes"`
	ClientCalls       sustainedIOStat `json:"client_calls"`
	WallSeconds       float64         `json:"wall_s_median,omitempty"`
}

// sustainedIOBudgetSet is everything frozen from the baseline arm.
type sustainedIOBudgetSet struct {
	FrozenAt      string                   `json:"frozen_at"`
	Arm           string                   `json:"arm"`
	Runs          []string                 `json:"runs"`
	BinarySHA256  string                   `json:"binary_sha256"`
	FixtureDigest string                   `json:"fixture_digest"`
	Phases        []sustainedIOPhaseBudget `json:"phases"`
	RetainedBytes sustainedIOStat          `json:"retained_bytes"`
...
const (
	...
	sustainedIOMainAdvanceFactor = 0.5
	// Retained storage after teardown, at most 1.2x the baseline's.
	sustainedIORetainedFactor = 1.2
	...
	sustainedIORegressionFactor = 1.10
)

var sustainedIOIdlePhases = map[string]bool{"P1_idle_cold": true, "P8_idle_warm": true}

// sustainedIOFreezeBudgets reduces the baseline arm to budgets.
//
...
func sustainedIOFreezeBudgets(runs []sustainedIORunArtifact) (sustainedIOBudgetSet, error) {
	if len(runs) == 0 {
		return sustainedIOBudgetSet{}, errors.New("no baseline runs to freeze budgets from")
	}
	set := sustainedIOBudgetSet{
	...
	for _, run := range runs {
		if run.Manifest.Arm != "baseline" {
			return sustainedIOBudgetSet{}, fmt.Errorf("run %s is arm %q; budgets are frozen from the baseline arm only", run.Dir, run.Manifest.Arm)
		}
		if run.Manifest.FixtureDigest != set.FixtureDigest {
			return sustainedIOBudgetSet{}, fmt.Errorf("run %s used fixture %s, not %s; a paired comparison needs one fixture", run.Dir, run.Manifest.FixtureDigest, set.FixtureDigest)
		}
	...
	for _, phase := range sustainedIOPhaseNames(runs) {
		budget := sustainedIOPhaseBudget{Phase: phase}
		labels := []string{}
		var resets, store, generations, sequence []*uint64
		for _, run := range runs {
			labels = append(labels, filepath.Base(run.Dir))
			report, found := sustainedIOFindPhase(run, phase)
			if !found {
				resets, store, generations, sequence =
					append(resets, nil), append(store, nil), append(generations, nil), append(sequence, nil)
				continue
			}
			resets = append(resets, sustainedIOUint64(uint64(max(report.WALResets, 0))))
			store = append(store, sustainedIOUint64(uint64(max(report.StoreBytesAfter, 0))))
			generations = append(generations, sustainedIOUint64(uint64(max(report.StoreCensusAfter.Generations.Max, 0))))
			sequence = append(sequence, sustainedIOUint64(uint64(max(report.StoreCensusAfter.Generations.Sequence, 0))))
		}
		...
		series := sustainedIOReducePhase(phase, runs)
		...
		budget.WALResets = sustainedIOStatistic(labels, resets)
		budget.StoreBytesAfter = sustainedIOStatistic(labels, store)
		budget.GenerationsMax = sustainedIOStatistic(labels, generations)
		budget.GenerationSeq = sustainedIOStatistic(labels, sequence)
		budget.WallSeconds = series.WallSeconds
		for _, window := range series.Windows {
			budget.Windows = append(budget.Windows, sustainedIOWindowBudget(window))
		}
		budget.JudgedSeries, budget.JudgedSource, budget.JudgedWall = sustainedIOBudgetJudgment(budget)
		budget.Limit, budget.LimitRule = sustainedIOLimitFor(phase, budget)
	...
	for _, run := range runs {
		...
		retained = append(retained, sustainedIOUint64(uint64(max(run.RetainedCensus.TotalBytes, 0))))
	}
	set.RetainedBytes = sustainedIOStatistic(retainedLabels, retained)
	if set.RetainedBytes.OK() {
		set.RetainedLimit = uint64(float64(set.RetainedBytes.Median) * sustainedIORetainedFactor)
	}
	...
	set.Digest = sustainedIOBudgetDigest(set)
	return set, nil
}

// sustainedIOCheckpointExcludedPhases are the phases judged on the checkpoint-excluded
...
var sustainedIOCheckpointExcludedPhases = map[string]bool{
	"P2_small_edits":         true,
...
const (
	sustainedIOSeriesTotal    = "ri_logical_writes"
	sustainedIOSeriesExcluded = "ri_logical_writes_excl_checkpoint"
)

// sustainedIOJudgedSeriesName is which write series a phase's ceiling is applied to.
func sustainedIOJudgedSeriesName(phase string) string {
	if sustainedIOCheckpointExcludedPhases[phase] {
		return sustainedIOSeriesExcluded
	}
	return sustainedIOSeriesTotal
}

// sustainedIOJudgedStat picks the series a phase is judged on out of a budget, falling
...
func sustainedIOJudgedStat(phase string, total, excluded sustainedIOStat) (sustainedIOStat, string) {
	if !sustainedIOCheckpointExcludedPhases[phase] {
		return total, sustainedIOSeriesTotal
	}
	if excluded.OK() {
		return excluded, sustainedIOSeriesExcluded
	}
	return total, sustainedIOSeriesTotal + " (the checkpoint-excluded series is unavailable for this phase)"
}
...
// sustainedIOJudgedSourcePhase is the source label of an arm judged on its phase row.
const sustainedIOJudgedSourcePhase = "phase"

// sustainedIOWindowStats is one arm's reading of one sub-window, in the shape the
...
type sustainedIOWindowStats struct {
	Window            string
	Present           bool
	LogicalWrites     sustainedIOStat
	LogicalWritesExcl sustainedIOStat
	WallSeconds       float64
}

// sustainedIOJudged is one arm's judged reading: which number, off which series, out of
// which row, over how long.
type sustainedIOJudged struct {
	Stat   sustainedIOStat
	Series string
	Source string
	Wall   float64
...
// sustainedIOSelectJudged picks the number one arm is judged on. It is the single place
...
func sustainedIOSelectJudged(phase string, total, excluded sustainedIOStat, wall float64, window sustainedIOWindowStats) sustainedIOJudged {
	mapped, hasMapping := sustainedIOJudgedWindows[phase]
	if hasMapping && window.Present && window.Window == mapped {
		stat, series := sustainedIOJudgedStat(phase, window.LogicalWrites, window.LogicalWritesExcl)
		if stat.OK() {
			...
			return sustainedIOJudged{Stat: stat, Series: series, Source: "window:" + mapped, Wall: judgedWall, Note: note}
		}
	}
	stat, series := sustainedIOJudgedStat(phase, total, excluded)
	judged := sustainedIOJudged{Stat: stat, Series: series, Source: sustainedIOJudgedSourcePhase, Wall: wall}
	if hasMapping {
		if window.Present {
...
// sustainedIOBudgetWindowStats lowers a frozen budget's sub-windows into the selection's
// shape.
func sustainedIOBudgetWindowStats(phase string, windows []sustainedIOWindowBudget) sustainedIOWindowStats {
	mapped, ok := sustainedIOJudgedWindows[phase]
	if !ok {
		return sustainedIOWindowStats{}
	}
	for _, window := range windows {
		if window.Window != mapped {
			continue
		}
		return sustainedIOWindowStats{Window: window.Window, Present: true,
			LogicalWrites: window.LogicalWrites, LogicalWritesExcl: window.LogicalWritesExcl,
			WallSeconds: window.WallSeconds}
	}
	return sustainedIOWindowStats{Window: mapped}
}

// sustainedIOSeriesWindowStats lowers a reduced arm's sub-windows into the same shape.
func sustainedIOSeriesWindowStats(phase string, windows []sustainedIOWindowSeries) sustainedIOWindowStats {
	mapped, ok := sustainedIOJudgedWindows[phase]
	if !ok {
		return sustainedIOWindowStats{}
	}
...
// sustainedIOBudgetJudged is the baseline half of the selection, off a frozen budget.
func sustainedIOBudgetJudged(budget sustainedIOPhaseBudget) sustainedIOJudged {
	return sustainedIOSelectJudged(budget.Phase, budget.LogicalWrites, budget.LogicalWritesExcl,
		budget.WallSeconds, sustainedIOBudgetWindowStats(budget.Phase, budget.Windows))
}

// sustainedIOBudgetJudgment is what a budget records about its own selection: the
...
// exactly as it did.
func sustainedIOBudgetJudgment(budget sustainedIOPhaseBudget) (string, string, float64) {
	selection := sustainedIOBudgetJudged(budget)
	if selection.Source == sustainedIOJudgedSourcePhase {
...
func sustainedIOLimitFor(phase string, budget sustainedIOPhaseBudget) (uint64, string) {
	...
	switch {
	case sustainedIOIdlePhases[phase]:
		absolute := uint64(float64(sustainedIOIdleBudgetPerMinute) * max(selection.Wall, 1) / 60)
		limit := baseline
		if absolute < limit {
			limit = absolute
		}
		return limit, fmt.Sprintf("idle on %s: min(baseline median %d, %d MiB per 60s scaled to %.0fs = %d)%s%s", series, baseline, sustainedIOIdleBudgetPerMinute>>20, selection.Wall, absolute, checkpoint, source)
	case phase == "P5_main_advance":
		return uint64(float64(baseline) * sustainedIOMainAdvanceFactor), fmt.Sprintf("headline on %s: %.2f x baseline median %d%s%s", series, sustainedIOMainAdvanceFactor, baseline, checkpoint, source)
	...
	default:
		return 0, fmt.Sprintf("no frozen ceiling; baseline %s median %d is recorded and a candidate above %.0f%% of it is preserved as a regression%s%s", series, baseline, sustainedIORegressionFactor*100, checkpoint, source)
	}
}

func sustainedIOPhaseNames(runs []sustainedIORunArtifact) []string {
...
func sustainedIOFindPhase(run sustainedIORunArtifact, phase string) (sustainedIOPhaseReport, bool) {
	for _, report := range run.Phases {
		if report.Phase == phase {
			return report, true
		}
	}
	return sustainedIOPhaseReport{}, false
}

func sustainedIOMedianFloat(values []float64) float64 {
	if len(values) == 0 {
		return 0
	}
...
// sustainedIOBudgetDigest is the canonical digest of a budget set, computed over the set
// with its own Digest field cleared.
func sustainedIOBudgetDigest(set sustainedIOBudgetSet) string {
	set.Digest = ""
...
// sustainedIOWriteFrozenBudgets writes budgets.json if it does not exist, and otherwise
...
func sustainedIOWriteFrozenBudgets(path string, set sustainedIOBudgetSet) (sustainedIOBudgetSet, bool, error) {
	if data, err := os.ReadFile(path); err == nil {
		var existing sustainedIOBudgetSet
		if err := json.Unmarshal(data, &existing); err != nil {
			return sustainedIOBudgetSet{}, false, fmt.Errorf("%s exists but is not a budget set: %w", path, err)
		}
		return existing, false, nil
	} else if !errors.Is(err, os.ErrNotExist) {
		return sustainedIOBudgetSet{}, false, err
	}
...
// sustainedIOStatEqual compares two reductions exactly, readings and named gaps alike.
func sustainedIOStatEqual(a, b sustainedIOStat) bool {
...
// sustainedIOAugmentFrozenBudgets fills the derived checkpoint series onto a budget set
...
// new one.
func sustainedIOAugmentFrozenBudgets(set sustainedIOBudgetSet, runs []sustainedIORunArtifact) (sustainedIOBudgetSet, error) {
	if digest := sustainedIOBudgetDigest(set); digest != set.Digest {
	...
	augmented := set
	augmented.Phases = append([]sustainedIOPhaseBudget(nil), set.Phases...)
	for i := range augmented.Phases {
		budget := &augmented.Phases[i]
		series := sustainedIOReducePhase(budget.Phase, runs)
		if !sustainedIOStatEqual(series.LogicalWrites, budget.LogicalWrites) {
		...
		budget.JudgedSeries, budget.JudgedSource, budget.JudgedWall = sustainedIOBudgetJudgment(*budget)
		budget.Limit, budget.LimitRule = sustainedIOLimitFor(budget.Phase, *budget)
	}
	...
	augmented.Digest = sustainedIOBudgetDigest(augmented)
	return augmented, nil
}

// ---------------------------------------------------------------- verdict ---

// sustainedIOPhaseVerdict is one phase's paired outcome.
type sustainedIOPhaseVerdict struct {
	Phase string `json:"phase"`
	...
	BaselineJudged      sustainedIOStat `json:"baseline_judged"`
	CandidateJudged     sustainedIOStat `json:"candidate_judged"`
	BaselineExcl        sustainedIOStat `json:"baseline_ri_logical_writes_excl_checkpoint"`
	CandidateExcl       sustainedIOStat `json:"candidate_ri_logical_writes_excl_checkpoint"`
	BaselineCheckpoint  sustainedIOStat `json:"baseline_wal_checkpoint_bytes"`
	CandidateCheckpoint sustainedIOStat `json:"candidate_wal_checkpoint_bytes"`
	...
	Windows []sustainedIOWindowVerdict `json:"windows,omitempty"`

	Baseline       sustainedIOStat `json:"baseline_ri_logical_writes"`
	Candidate      sustainedIOStat `json:"candidate_ri_logical_writes"`
	BaselineDisk   sustainedIOStat `json:"baseline_ri_diskio_byteswritten"`
	CandidateDisk  sustainedIOStat `json:"candidate_ri_diskio_byteswritten"`
	BaselineWAL    sustainedIOStat `json:"baseline_wal_bytes_after"`
	CandidateWAL   sustainedIOStat `json:"candidate_wal_bytes_after"`
...
// sustainedIOWindowVerdict is one sub-window's paired row. Sub-windows exist so a
...
type sustainedIOWindowVerdict struct {
	Window              string          `json:"window"`
	Detail              string          `json:"detail,omitempty"`
	Baseline            sustainedIOStat `json:"baseline_ri_logical_writes"`
	Candidate           sustainedIOStat `json:"candidate_ri_logical_writes"`
	...
	BaselineCheckpoint  sustainedIOStat `json:"baseline_wal_checkpoint_bytes"`
	CandidateCheckpoint sustainedIOStat `json:"candidate_wal_checkpoint_bytes"`
	BaselineCalls       sustainedIOStat `json:"baseline_client_calls"`
	CandidateCalls      sustainedIOStat `json:"candidate_client_calls"`
	Ratio               float64         `json:"ratio,omitempty"`
	Note                string          `json:"note"`
}

// sustainedIOVerdict is the paired outcome of one measurement.
type sustainedIOVerdict struct {
	...
	Phases            []sustainedIOPhaseVerdict `json:"phases"`
	RetainedBaseline  sustainedIOStat           `json:"retained_bytes_baseline"`
	RetainedCandidate sustainedIOStat           `json:"retained_bytes_candidate"`
	RetainedLimit     uint64                    `json:"retained_bytes_limit"`
	...
	Unbudgeted []sustainedIOPhaseVerdict `json:"unbudgeted_phases,omitempty"`
}

// sustainedIOCompareArms judges the candidate against frozen budgets.
//
...
func sustainedIOCompareArms(set sustainedIOBudgetSet, candidate []sustainedIORunArtifact) (sustainedIOVerdict, error) {
	if len(candidate) == 0 {
		return sustainedIOVerdict{}, errors.New("no candidate runs to compare")
	}
	verdict := sustainedIOVerdict{
	...
	for _, run := range candidate {
		if run.Manifest.Arm != "candidate" {
			return sustainedIOVerdict{}, fmt.Errorf("run %s is arm %q, not the candidate", run.Dir, run.Manifest.Arm)
		}
		if set.FixtureDigest != "" && run.Manifest.FixtureDigest != set.FixtureDigest {
			return sustainedIOVerdict{}, fmt.Errorf("run %s used fixture %s; the budgets were frozen over %s", run.Dir, run.Manifest.FixtureDigest, set.FixtureDigest)
		}
	...
	if digest := sustainedIOBudgetDigest(set); digest != set.Digest {
		return sustainedIOVerdict{}, fmt.Errorf("budget set digest %s does not match its contents %s; the frozen budgets were edited", set.Digest, digest)
	}

	budgeted := map[string]bool{}
	for _, budget := range set.Phases {
		budgeted[budget.Phase] = true
		row := sustainedIOPhaseVerdict{
		...
		candidateSeries := sustainedIOFillCandidateSide(&row, budget.Phase, candidate, budget.Windows)

		baselineSelection := sustainedIOBudgetJudged(budget)
		candidateSelection := sustainedIOSelectJudged(budget.Phase, row.Candidate, row.CandidateExcl,
			candidateSeries.WallSeconds, sustainedIOSeriesWindowStats(budget.Phase, candidateSeries.Windows))
		...
		row.JudgedNote = sustainedIOJoinJudgedNotes(baselineSelection.Note, candidateSelection.Note)
		...
		default:
			...
			row.Regressed = row.Ratio > sustainedIORegressionFactor
		}
		if !row.Comparable {
	...
	for _, phase := range sustainedIOPhaseNames(candidate) {
		if budgeted[phase] {
			continue
		}
		row := sustainedIOPhaseVerdict{Phase: phase, JudgedSeries: sustainedIOJudgedSeriesName(phase),
			LimitRule: "no frozen budget names this phase; recorded as evidence and not judged"}
		series := sustainedIOFillCandidateSide(&row, phase, candidate, nil)
		selection := sustainedIOSelectJudged(phase, row.Candidate, row.CandidateExcl,
			series.WallSeconds, sustainedIOSeriesWindowStats(phase, series.Windows))
	...
	verdict.RetainedCandidate = sustainedIOStatistic(retainedLabels, retained)
	...
	if verdict.RetainedCandidate.OK() && verdict.RetainedLimit > 0 && !verdict.RetainedOK {
		verdict.OverBudget = append(verdict.OverBudget, fmt.Sprintf("retained_bytes: %d > %d (%.1fx baseline)", verdict.RetainedCandidate.Median, verdict.RetainedLimit, sustainedIORetainedFactor))
	}
	return verdict, nil
}

// sustainedIOWindowSeries is one sub-window's reduction across an arm's repetitions.
type sustainedIOWindowSeries struct {
	Window            string
	Detail            string
	LogicalWrites     sustainedIOStat
	LogicalWritesExcl sustainedIOStat
	CheckpointBytes   sustainedIOStat
	ClientCalls       sustainedIOStat
	WallSeconds       float64
}

// sustainedIOPhaseSeries is one phase's reduction across an arm's repetitions. Both
...
type sustainedIOPhaseSeries struct {
	Detail            string
	LogicalWrites     sustainedIOStat
	LogicalWritesExcl sustainedIOStat
	CheckpointBytes   sustainedIOStat
	DiskWritten       sustainedIOStat
	WALBytesAfter     sustainedIOStat
	ClientCalls       sustainedIOStat
	WallSeconds       float64
	Counters          map[string]int64
	Windows           []sustainedIOWindowSeries
}

// sustainedIOReducePhase reduces one phase across one arm's runs.
func sustainedIOReducePhase(phase string, runs []sustainedIORunArtifact) sustainedIOPhaseSeries {
	series := sustainedIOPhaseSeries{Counters: map[string]int64{}}
	labels := []string{}
	var logical, excluded, checkpoint, disk, wal, calls []*uint64
	wall := []float64{}
	windowNames := []string{}
	seenWindow := map[string]bool{}
	windowValues := map[string]*sustainedIOWindowSeries{}
	...
	for _, run := range runs {
		...
		if report.LogicalWrites == nil {
			...
			checkpoint = append(checkpoint, sustainedIOUint64(report.CheckpointBytes))
		}
		disk = append(disk, sustainedIOUint64(report.DiskWritten))
		wal = append(wal, sustainedIOUint64(uint64(max(report.WALAfter, 0))))
		calls = append(calls, sustainedIOUint64(uint64(max(report.ClientCalls, 0))))
		...
		for _, window := range report.Windows {
			if !seenWindow[window.Window] {
				...
				windowValues[window.Window] = &sustainedIOWindowSeries{Window: window.Window, Detail: window.Detail}
			...
			if window.LogicalWrites == nil {
				...
				windowCheckpoint[window.Window] = append(windowCheckpoint[window.Window], sustainedIOUint64(window.CheckpointBytes))
			}
			windowCalls[window.Window] = append(windowCalls[window.Window], sustainedIOUint64(uint64(max(window.ClientCalls, 0))))
	...
	series.LogicalWrites = sustainedIOStatistic(labels, logical)
	series.LogicalWritesExcl = sustainedIOStatistic(labels, excluded)
	series.CheckpointBytes = sustainedIOStatistic(labels, checkpoint)
	series.DiskWritten = sustainedIOStatistic(labels, disk)
	series.WALBytesAfter = sustainedIOStatistic(labels, wal)
	series.ClientCalls = sustainedIOStatistic(labels, calls)
	series.WallSeconds = sustainedIOMedianFloat(wall)
	for _, name := range windowNames {
		window := windowValues[name]
		window.LogicalWrites = sustainedIOStatistic(labels, windowLogical[name])
		window.LogicalWritesExcl = sustainedIOStatistic(labels, windowExcluded[name])
		window.CheckpointBytes = sustainedIOStatistic(labels, windowCheckpoint[name])
		window.ClientCalls = sustainedIOStatistic(labels, windowCalls[name])
		window.WallSeconds = sustainedIOMedianFloat(windowWall[name])
		series.Windows = append(series.Windows, *window)
	}
	return series
}

// sustainedIOFillCandidateSide fills the candidate half of a phase row, including every
// sub-window either arm recorded.
func sustainedIOFillCandidateSide(row *sustainedIOPhaseVerdict, phase string, candidate []sustainedIORunArtifact, baselineWindows []sustainedIOWindowBudget) sustainedIOPhaseSeries {
	series := sustainedIOReducePhase(phase, candidate)
	...
	row.CounterSummary = sustainedIOFormatCounters(series.Counters)

	baseline := map[string]sustainedIOWindowBudget{}
	for _, window := range baselineWindows {
		baseline[window.Window] = window
	}
	judgedWindow := sustainedIOJudgedWindows[phase]
	for _, window := range series.Windows {
		...
		verdictRow := sustainedIOWindowVerdict{
...
// sustainedIOFormatCounters renders the reuse counters a phase moved, largest first, so
// a write reduction can be read next to the reuse that produced it.
func sustainedIOFormatCounters(counters map[string]int64) string {
	if len(counters) == 0 {
		return ""
	}
...
// sustainedIORenderVerdictTable renders the verdict as the markdown table the
...
func sustainedIORenderVerdictTable(verdict sustainedIOVerdict) string {
	...
	out.WriteString("|---|---|---|---|---|---|---|---|---|---|---|---|---|\n")
	rows := append(append([]sustainedIOPhaseVerdict{}, verdict.Phases...), verdict.Unbudgeted...)
	for _, row := range rows {
		...
		fmt.Fprintf(&out, "| %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n",
			row.Phase, row.JudgedSeries, sustainedIOFormatJudgedRows(row), sustainedIOFormatStat(baselineJudged), sustainedIOFormatStat(candidateJudged), ratio, totalRatio,
			sustainedIOFormatStat(row.BaselineCheckpoint), sustainedIOFormatStat(row.CandidateCheckpoint), ceiling, state,
			sustainedIOFormatStat(row.BaselineDisk), sustainedIOFormatStat(row.CandidateDisk))
	}
	windows := sustainedIORenderWindowTable(rows)
	if windows != "" {
		out.WriteString("\n")
...
// sustainedIORenderWindowTable renders the sub-window rows. They carry no ceiling and
// the table says so in every row rather than once in a caption.
func sustainedIORenderWindowTable(rows []sustainedIOPhaseVerdict) string {
	var out strings.Builder
	any := false
	for _, row := range rows {
		for _, window := range row.Windows {
			...
			fmt.Fprintf(&out, "| %s | %s | %s | %s | %s | %s | %s | %s |\n",
				row.Phase, window.Window, sustainedIOFormatStat(window.Baseline), sustainedIOFormatStat(window.Candidate),
				sustainedIOFormatStat(window.CandidateExcl), sustainedIOFormatStat(window.CandidateCheckpoint),
				sustainedIOFormatStat(window.CandidateCalls), window.Note)
		}
	}
	return out.String()
}

// sustainedIOJoinJudgedNotes carries both arms' selection notes on one row, and says a
// thing once when it is true of both arms.
func sustainedIOJoinJudgedNotes(baseline, candidate string) string {
	switch {
...
// sustainedIOFormatJudgedRows renders which row each arm was judged off, with the wall
...
func sustainedIOFormatJudgedRows(row sustainedIOPhaseVerdict) string {
...
func sustainedIOFormatStat(stat sustainedIOStat) string {
	if !stat.OK() {
...
// TestSustainedIOPairedArmsVerdict is the reduction step of the paired protocol, run
...
func TestSustainedIOPairedArmsVerdict(t *testing.T) {
	dir := os.Getenv("GX_SUSTAINED_IO_PAIRED_ARTIFACT_DIR")
	if dir == "" {
		t.Skip("set GX_SUSTAINED_IO_PAIRED_ARTIFACT_DIR to reduce a completed paired run into frozen budgets and a verdict")
	}
	baseline, err := sustainedIOLoadArmRuns(dir, "baseline")
	if err != nil {
		t.Fatal(err)
	}
	frozen, err := sustainedIOFreezeBudgets(baseline)
	if err != nil {
		t.Fatal(err)
	}
	budgetsPath := filepath.Join(dir, "budgets.json")
	frozen, wrote, err := sustainedIOWriteFrozenBudgets(budgetsPath, frozen)
	if err != nil {
		t.Fatal(err)
	}
	...
	// refusal in sustainedIOAugmentFrozenBudgets. The file on disk is not rewritten.
	frozen, err = sustainedIOAugmentFrozenBudgets(frozen, baseline)
	if err != nil {
		t.Fatal(err)
	}
	...
	if os.Getenv("GX_SUSTAINED_IO_BUDGETS_ONLY") != "" {
		t.Log("GX_SUSTAINED_IO_BUDGETS_ONLY is set; the candidate arm was not read")
		return
	}
	candidate, err := sustainedIOLoadArmRuns(dir, "candidate")
	if err != nil {
		t.Fatal(err)
	}
	verdict, err := sustainedIOCompareArms(frozen, candidate)
	if err != nil {
		t.Fatal(err)
	}
	sustainedIOWriteJSON(t, filepath.Join(dir, "verdict.json"), verdict)
	table := sustainedIORenderVerdictTable(verdict)
...
func TestSustainedIOStatisticNamesAMissingReadingInsteadOfCountingItAsZero(t *testing.T) {
	stat := sustainedIOStatistic([]string{"rep1", "rep2", "rep3"}, []*uint64{sustainedIOUint64(100), nil, sustainedIOUint64(300)})
	if stat.N != 2 {
	...
	if empty := sustainedIOStatistic(nil, []*uint64{nil, nil}); empty.OK() || len(empty.Unavailable) != 2 {
		t.Fatalf("a series with no readings must not report OK: %+v", empty)
	}
	odd := sustainedIOStatistic(nil, []*uint64{sustainedIOUint64(5), sustainedIOUint64(1), sustainedIOUint64(3)})
	if odd.Median != 3 || odd.Min != 1 || odd.Max != 5 {
		t.Fatalf("three readings reduced to %+v", odd)
	}
}

// sustainedIOSyntheticRun builds one arm-repetition artifact for the reduction tests.
func sustainedIOSyntheticRun(arm string, logical map[string]uint64, retained int64) sustainedIORunArtifact {
	run := sustainedIORunArtifact{
		Manifest:       sustainedIOManifest{Arm: arm, BinarySHA256: arm + "-sha", FixtureDigest: "fixture-1"},
		RetainedCensus: sustainedIOCensus{TotalBytes: retained},
	}
	for _, phase := range []string{"P1_idle_cold", "P2_small_edits", "P5_main_advance"} {
		value, ok := logical[phase]
		report := sustainedIOPhaseReport{Phase: phase, Detail: phase + " detail", WallSeconds: 60, WALAfter: 4096, StoreBytesAfter: 1 << 20}
		if ok {
			report.LogicalWrites = sustainedIOUint64(value)
...
func TestSustainedIOFreezeBudgetsRefusesAnythingButTheBaselineArm(t *testing.T) {
	baseline := []sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 1000, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 100),
		sustainedIOSyntheticRun("candidate", map[string]uint64{"P1_idle_cold": 1, "P2_small_edits": 2, "P5_main_advance": 3}, 100),
	}
	if _, err := sustainedIOFreezeBudgets(baseline); err == nil {
		t.Fatal("budgets were frozen from a set containing a candidate run; the candidate's numbers must never reach a budget")
	}
	if _, err := sustainedIOFreezeBudgets(nil); err == nil {
		t.Fatal("budgets were frozen from no runs at all")
	}
	mixed := []sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P2_small_edits": 2000}, 100),
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P2_small_edits": 2000}, 100),
	}
	mixed[1].Manifest.FixtureDigest = "fixture-2"
	if _, err := sustainedIOFreezeBudgets(mixed); err == nil {
		t.Fatal("budgets were frozen across two different fixtures; the arms would not be paired")
	}
}

func TestSustainedIOFreezeBudgetsAppliesThePlansCeilings(t *testing.T) {
	runs := []sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 30 << 20, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 100),
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 30 << 20, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 100),
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 30 << 20, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 100),
	}
	set, err := sustainedIOFreezeBudgets(runs)
	if err != nil {
		t.Fatal(err)
	}
	...
	if limits["P1_idle_cold"] != sustainedIOIdleBudgetPerMinute {
		t.Fatalf("idle ceiling = %d, want the %d-byte absolute cap", limits["P1_idle_cold"], sustainedIOIdleBudgetPerMinute)
	}
	...
	if set.Digest == "" || set.Digest != sustainedIOBudgetDigest(set) {
		t.Fatal("the budget set does not carry a digest of its own contents")
	}
}

func TestSustainedIOFrozenBudgetsRefuseToBeRewrittenOnceTheCandidateHasBeenSeen(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "budgets.json")
	first, err := sustainedIOFreezeBudgets([]sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P5_main_advance": 8_000_000}, 100),
	})
	if err != nil {
		t.Fatal(err)
	}
	stored, wrote, err := sustainedIOWriteFrozenBudgets(path, first)
	if err != nil || !wrote {
	...
	looser, err := sustainedIOFreezeBudgets([]sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P5_main_advance": 800_000_000}, 100),
	})
	if err != nil {
		t.Fatal(err)
	}
	back, wrote, err := sustainedIOWriteFrozenBudgets(path, looser)
	if err != nil {
		t.Fatal(err)
	}
	if wrote {
...
func TestSustainedIOCompareArmsJudgesAgainstTheFrozenCeilingsAndPreservesRegressions(t *testing.T) {
	baseline := []sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 1 << 20, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 1000),
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 1 << 20, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 1000),
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P1_idle_cold": 1 << 20, "P2_small_edits": 2000, "P5_main_advance": 8_000_000}, 1000),
	}
	set, err := sustainedIOFreezeBudgets(baseline)
	if err != nil {
		t.Fatal(err)
	}
	candidate := []sustainedIORunArtifact{
		// idle within budget, edits regressed 50%, main advance halved.
		sustainedIOSyntheticRun("candidate", map[string]uint64{"P1_idle_cold": 1 << 19, "P2_small_edits": 3000, "P5_main_advance": 3_000_000}, 1100),
		sustainedIOSyntheticRun("candidate", map[string]uint64{"P1_idle_cold": 1 << 19, "P2_small_edits": 3000, "P5_main_advance": 3_000_000}, 1100),
		sustainedIOSyntheticRun("candidate", map[string]uint64{"P1_idle_cold": 1 << 19, "P2_small_edits": 3000, "P5_main_advance": 3_000_000}, 1100),
	}
	verdict, err := sustainedIOCompareArms(set, candidate)
	if err != nil {
		t.Fatal(err)
	}
	rows := map[string]sustainedIOPhaseVerdict{}
...
func TestSustainedIOCompareArmsRefusesEditedBudgetsAndUnpairedFixtures(t *testing.T) {
	set, err := sustainedIOFreezeBudgets([]sustainedIORunArtifact{
		sustainedIOSyntheticRun("baseline", map[string]uint64{"P5_main_advance": 8_000_000}, 100),
	})
	if err != nil {
		t.Fatal(err)
	}
	candidate := []sustainedIORunArtifact{sustainedIOSyntheticRun("candidate", map[string]uint64{"P5_main_advance": 1_000_000}, 100)}

	edited := set
	edited.Phases = append([]sustainedIOPhaseBudget(nil), set.Phases...)
	edited.Phases[0].Limit = 1 << 40
	if _, err := sustainedIOCompareArms(edited, candidate); err == nil {
		t.Fatal("a verdict was produced against budgets whose contents no longer match their digest")
	}
	unpaired := []sustainedIORunArtifact{sustainedIOSyntheticRun("candidate", map[string]uint64{"P5_main_advance": 1}, 100)}
	unpaired[0].Manifest.FixtureDigest = "fixture-9"
	if _, err := sustainedIOCompareArms(set, unpaired); err == nil {
		t.Fatal("a verdict was produced across two different fixtures")
	}
	if _, err := sustainedIOCompareArms(set, []sustainedIORunArtifact{sustainedIOSyntheticRun("baseline", nil, 100)}); err == nil {
		t.Fatal("a baseline run was accepted as the candidate arm")
	}
}

func TestSustainedIOCompareArmsNeverPassesAPhaseItCannotCompare(t *testing.T) {
	// The baseline lost its process counters for the headline phase.
	blind := sustainedIOSyntheticRun("baseline", nil, 100)
	set, err := sustainedIOFreezeBudgets([]sustainedIORunArtifact{blind})
	if err != nil {
		t.Fatal(err)
	}
	for _, budget := range set.Phases {
...
func TestSustainedIOLoadArmRunsReadsEveryRepetitionInOrder(t *testing.T) {
	dir := t.TempDir()
	for i, value := range []uint64{300, 100, 200} {
		...
		run := sustainedIOSyntheticRun("baseline", map[string]uint64{"P5_main_advance": value}, int64(value))
	...
	runs, err := sustainedIOLoadArmRuns(dir, "baseline")
	if err != nil {
		t.Fatal(err)
	}
	if len(runs) != 3 {
...
// sustainedIOCheckpointRun builds one arm-repetition artifact carrying both write
// series, so the reduction can be exercised on the exact numbers the frozen
// paired1500 artifacts hold.
func sustainedIOCheckpointRun(arm string, totals, checkpoints map[string]uint64) sustainedIORunArtifact {
	run := sustainedIORunArtifact{
		Manifest:       sustainedIOManifest{Arm: arm, BinarySHA256: arm + "-sha", FixtureDigest: "fixture-1"},
		RetainedCensus: sustainedIOCensus{TotalBytes: 1024},
	}
	for _, phase := range []string{"P2_small_edits", "P5_main_advance"} {
		...
		if ok {
			...
			report.LogicalWritesExcl = sustainedIOExcludeCheckpoint(&value, report.CheckpointBytes)
		}
		run.Phases = append(run.Phases, report)
	}
	return run
...
// sustainedIOIdleArtifact builds one arm-repetition carrying an idle phase, optionally
...
func sustainedIOIdleArtifact(arm, phase string, quiet, polling uint64, split bool, idleWall float64) sustainedIORunArtifact {
	value := func(v uint64) *uint64 { return &v }
	run := sustainedIOCheckpointRun(arm, map[string]uint64{"P2_small_edits": 10, "P5_main_advance": 20}, map[string]uint64{})
	report := sustainedIOPhaseReport{Phase: phase, Detail: "idle", WallSeconds: idleWall}
	if !split {
		total := polling
	...
	quietName, pollingName := sustainedIOWindowIdleColdQuiet, sustainedIOWindowIdleColdPolling
	if phase == "P8_idle_warm" {
		quietName, pollingName = sustainedIOWindowIdleWarmQuiet, sustainedIOWindowIdleWarmPolling
	}
	...
	// opened.
	report.Windows = []sustainedIOWindowReport{
...
// TestSustainedIOIdlePhaseIsJudgedOnThePollingArmAgainstTheFrozenPhase is the revert-red
...
func TestSustainedIOIdlePhaseIsJudgedOnThePollingArmAgainstTheFrozenPhase(t *testing.T) {
	// The frozen baseline: measured before the split, so it carries no windows.
	baseline := []sustainedIORunArtifact{sustainedIOIdleArtifact("baseline", "P8_idle_warm", 0, 3_299_312, false, 62)}
	set, err := sustainedIOFreezeBudgets(baseline)
	if err != nil {
		t.Fatal(err)
	}
	budget := sustainedIOFindBudget(t, set, "P8_idle_warm")
	...
	candidate := []sustainedIORunArtifact{sustainedIOIdleArtifact("candidate", "P8_idle_warm", 3_000_000, 3_100_000, true, 60)}
	verdict, err := sustainedIOCompareArms(set, candidate)
	if err != nil {
		t.Fatal(err)
	}
	row := sustainedIOFindVerdict(t, verdict, "P8_idle_warm")
	if row.CandidateSource != "window:"+sustainedIOWindowIdleWarmPolling {
		t.Fatalf("the candidate was judged off %q, not its polling arm", row.CandidateSource)
	}
	if row.BaselineSource != sustainedIOJudgedSourcePhase {
	...
	quiet, polling := sustainedIOFindWindowRow(t, row, sustainedIOWindowIdleWarmQuiet), sustainedIOFindWindowRow(t, row, sustainedIOWindowIdleWarmPolling)
	...
	table := sustainedIORenderVerdictTable(verdict)
	if !strings.Contains(table, "window:"+sustainedIOWindowIdleWarmPolling) || !strings.Contains(table, "phase @62s") {
...
// TestSustainedIOFrozenIdleArtifactsStayJudgedOnThePhaseOnBothArms is the other half of
...
func TestSustainedIOFrozenIdleArtifactsStayJudgedOnThePhaseOnBothArms(t *testing.T) {
	set, err := sustainedIOFreezeBudgets([]sustainedIORunArtifact{sustainedIOIdleArtifact("baseline", "P1_idle_cold", 0, 1_490_944, false, 62)})
	if err != nil {
		t.Fatal(err)
	}
	verdict, err := sustainedIOCompareArms(set, []sustainedIORunArtifact{sustainedIOIdleArtifact("candidate", "P1_idle_cold", 0, 1_388_592, false, 62)})
	if err != nil {
		t.Fatal(err)
	}
	row := sustainedIOFindVerdict(t, verdict, "P1_idle_cold")
	if row.BaselineSource != sustainedIOJudgedSourcePhase || row.CandidateSource != sustainedIOJudgedSourcePhase {
	...
	if !strings.Contains(row.JudgedNote, "carries no "+sustainedIOWindowIdleColdPolling+" sub-window") {
		t.Fatalf("the row does not say why it was judged on the phase: %q", row.JudgedNote)
	}
}

// TestSustainedIOCompareArmsRefusesToJudgeTwoArmsOnDifferentSeries pins the asymmetry the
...
func TestSustainedIOCompareArmsRefusesToJudgeTwoArmsOnDifferentSeries(t *testing.T) {
	baseline := []sustainedIORunArtifact{sustainedIOCheckpointRun("baseline",
		map[string]uint64{"P2_small_edits": 16_879_664, "P5_main_advance": 100}, map[string]uint64{})}
	set, err := sustainedIOFreezeBudgets(baseline)
	if err != nil {
		t.Fatal(err)
	}
	candidate := sustainedIOCheckpointRun("candidate",
	...
	verdict, err := sustainedIOCompareArms(set, []sustainedIORunArtifact{candidate})
	if err != nil {
		t.Fatal(err)
	}
	row := sustainedIOFindVerdict(t, verdict, "P2_small_edits")
	if row.Comparable {
...
// TestSustainedIOLimitForIdleScalesByTheJudgedWindowWall pins the ceiling's duration
...
func TestSustainedIOLimitForIdleScalesByTheJudgedWindowWall(t *testing.T) {
	budget := sustainedIOPhaseBudget{
		Phase:         "P8_idle_warm",
		WallSeconds:   120,
		LogicalWrites: sustainedIOStatistic(nil, []*uint64{sustainedIOUint64(100 << 20)}),
		Windows: []sustainedIOWindowBudget{
			{Window: sustainedIOWindowIdleWarmQuiet, WallSeconds: 60, LogicalWrites: sustainedIOStatistic(nil, []*uint64{sustainedIOUint64(1 << 20)})},
			{Window: sustainedIOWindowIdleWarmPolling, WallSeconds: 60, LogicalWrites: sustainedIOStatistic(nil, []*uint64{sustainedIOUint64(50 << 20)})},
		},
	}
	limit, rule := sustainedIOLimitFor("P8_idle_warm", budget)
	if limit != sustainedIOIdleBudgetPerMinute {
		t.Fatalf("limit = %d, want the 8 MiB/60s absolute scaled by the judged window's own 60s", limit)
	}
	if !strings.Contains(rule, "scaled to 60s") || !strings.Contains(rule, "window:"+sustainedIOWindowIdleWarmPolling) {
	...
	if _, rule := sustainedIOLimitFor("P8_idle_warm", budget); !strings.Contains(rule, "scaled to 120s") {
		t.Fatalf("a window without a wall must fall back to the phase's: %q", rule)
	}
}

func sustainedIOFindWindowRow(t *testing.T, row sustainedIOPhaseVerdict, window string) sustainedIOWindowVerdict {
	t.Helper()
	...
	return sustainedIOWindowVerdict{}
}

// TestSustainedIOPairedVerdictReadsP2OnTheCheckpointExcludedSeries is the revert-red of
// the checkpoint-excluded-series correction, on the frozen paired1500 numbers.
//
...
func TestSustainedIOPairedVerdictReadsP2OnTheCheckpointExcludedSeries(t *testing.T) {
	baseline := []sustainedIORunArtifact{sustainedIOCheckpointRun("baseline",
	...
	p2 := sustainedIOFindBudget(t, set, "P2_small_edits")
	if p2.JudgedSeries != sustainedIOSeriesExcluded {
	...
	if p5 := sustainedIOFindBudget(t, set, "P5_main_advance"); p5.JudgedSeries != sustainedIOSeriesTotal {
		t.Fatalf("P5 is judged on %q; a continuously-writing phase keeps the total series", p5.JudgedSeries)
	}

	candidate := []sustainedIORunArtifact{sustainedIOCheckpointRun("candidate",
	...
	if !strings.Contains(sustainedIORenderVerdictTable(verdict), "42607016") {
		t.Fatal("the rendered table does not carry the checkpoint row")
	}
}

func sustainedIOFindBudget(t *testing.T, set sustainedIOBudgetSet, phase string) sustainedIOPhaseBudget {
	t.Helper()
	for _, budget := range set.Phases {
	...
	return sustainedIOPhaseBudget{}
}

func sustainedIOFindVerdict(t *testing.T, verdict sustainedIOVerdict, phase string) sustainedIOPhaseVerdict {
	t.Helper()
	for _, row := range verdict.Phases {
		if row.Phase == phase {
			return row
	...
	return sustainedIOPhaseVerdict{}
}

// TestSustainedIOExcludeCheckpointNeverInventsANegativeSeries pins the one arithmetic
...
func TestSustainedIOExcludeCheckpointNeverInventsANegativeSeries(t *testing.T) {
	if sustainedIOExcludeCheckpoint(nil, 10) != nil {
		t.Fatal("a missing total must stay missing, not become a zero")
	}
	total := uint64(100)
	if got := sustainedIOExcludeCheckpoint(&total, 400); got == nil || *got != 0 {
		t.Fatalf("excluded = %v, want 0", got)
	}
	if got := sustainedIOExcludeCheckpoint(&total, 40); got == nil || *got != 60 {
		t.Fatalf("excluded = %v, want 60", got)
	}
}

// TestSustainedIOFillCheckpointSeriesRederivesFromTheRunsOwnSamples is what makes the
...
func TestSustainedIOFillCheckpointSeriesRederivesFromTheRunsOwnSamples(t *testing.T) {
	dir := t.TempDir()
	value := func(v uint64) *uint64 { return &v }
	stream := []sustainedIOSample{
	...
	run := sustainedIORunArtifact{Dir: dir, Phases: []sustainedIOPhaseReport{
	...
	measured := sustainedIORunArtifact{Dir: dir, Phases: []sustainedIOPhaseReport{
		{Phase: "P2_small_edits", LogicalWrites: value(10), LogicalWritesExcl: value(7), CheckpointBytes: 3},
	}}
	sustainedIOFillCheckpointSeries(&measured)
...
// TestSustainedIOBudgetDigestSurvivesTheAddedSeriesWhenAPhaseCarriesNone is the guard
// that keeps every already-frozen budgets.json valid.
//
// sustainedIOCompareArms recomputes the digest of the set it is handed and refuses it if
// the bytes do not match. A field added to sustainedIOPhaseBudget that marshals even
...
func TestSustainedIOBudgetDigestSurvivesTheAddedSeriesWhenAPhaseCarriesNone(t *testing.T) {
	set := sustainedIOBudgetSet{
		FrozenAt: "2026-09-13T00:00:00Z", Arm: "baseline", Runs: []string{"baseline_rep1"},
		BinarySHA256: "sha", FixtureDigest: "fixture-1",
		Phases: []sustainedIOPhaseBudget{{
			Phase:         "P2_small_edits",
			LogicalWrites: sustainedIOStatistic([]string{"baseline_rep1"}, []*uint64{sustainedIOUint64(16_879_664)}),
	...
	if got := sustainedIOBudgetDigest(set); got != golden && os.Getenv("GX_SUSTAINED_IO_ACCEPT_GOLDEN") == "" {
		t.Fatalf("the canonical digest of a pre-checkpoint budget set changed: %s (golden %s); re-run with GX_SUSTAINED_IO_ACCEPT_GOLDEN=1 only after confirming no frozen budgets.json exists", got, golden)
	}
}

// TestSustainedIOAugmentFrozenBudgetsFillsTheDerivedHalfAndRefusesForeignRuns pins the
...
func TestSustainedIOAugmentFrozenBudgetsFillsTheDerivedHalfAndRefusesForeignRuns(t *testing.T) {
	runs := []sustainedIORunArtifact{sustainedIOCheckpointRun("baseline",
		map[string]uint64{"P2_small_edits": 16_879_664, "P5_main_advance": 5_147_295_504},
		map[string]uint64{"P2_small_edits": 24_752})}
	runs[0].Dir = "baseline_rep1"
	// A set frozen without the derived half: strip it.
	frozen, err := sustainedIOFreezeBudgets(runs)
	if err != nil {
		t.Fatal(err)
	}
	for i := range frozen.Phases {
		frozen.Phases[i].LogicalWritesExcl = sustainedIOStat{}
		frozen.Phases[i].CheckpointBytes = sustainedIOStat{}
		frozen.Phases[i].JudgedSeries = ""
		frozen.Phases[i].Limit, frozen.Phases[i].LimitRule = sustainedIOLimitFor(frozen.Phases[i].Phase, frozen.Phases[i])
	}
	frozen.Digest = sustainedIOBudgetDigest(frozen)
	before := sustainedIOFindBudget(t, frozen, "P2_small_edits")
	if before.Limit != 16_879_664 {
		t.Fatalf("the stripped set's P2 ceiling = %d", before.Limit)
	}

	augmented, err := sustainedIOAugmentFrozenBudgets(frozen, runs)
	if err != nil {
		t.Fatal(err)
	}
	after := sustainedIOFindBudget(t, augmented, "P2_small_edits")
	...
	if after.Limit != 16_854_912 || after.JudgedSeries != sustainedIOSeriesExcluded {
	...
	if _, err := sustainedIOCompareArms(augmented, []sustainedIORunArtifact{sustainedIOCheckpointRun("candidate",
	...
	foreign := []sustainedIORunArtifact{sustainedIOCheckpointRun("baseline",
		map[string]uint64{"P2_small_edits": 99, "P5_main_advance": 5_147_295_504}, map[string]uint64{})}
	foreign[0].Dir = "baseline_rep1"
	if _, err := sustainedIOAugmentFrozenBudgets(frozen, foreign); err == nil {
		t.Fatal("a baseline whose totals do not reproduce must be refused")
	}
	if _, err := sustainedIOAugmentFrozenBudgets(frozen, []sustainedIORunArtifact{sustainedIOCheckpointRun("candidate", nil, nil)}); err == nil {
	...
	again, err := sustainedIOAugmentFrozenBudgets(augmented, runs)
	if err != nil || again.Digest != augmented.Digest {
		t.Fatalf("augmenting twice moved the set: err=%v", err)
	}
}

// TestSustainedIOCompareArmsRecordsPhasesNoFrozenBudgetNames keeps a workload that
// gained a phase after the freeze honest: the phase cannot be judged, and it
// must not disappear either.
func TestSustainedIOCompareArmsRecordsPhasesNoFrozenBudgetNames(t *testing.T) {
	baseline := []sustainedIORunArtifact{sustainedIOCheckpointRun("baseline",
	...
	extra := uint64(4242)
	candidate.Phases = append(candidate.Phases, sustainedIOPhaseReport{
	...
	if !strings.Contains(sustainedIORenderVerdictTable(verdict), "P9_new_phase") {
		t.Fatal("the rendered table dropped the unbudgeted phase")
	}
}

// TestSustainedIOReducePhaseCarriesSubWindowRowsThroughTheVerdict pins that a phase's
...
func TestSustainedIOReducePhaseCarriesSubWindowRowsThroughTheVerdict(t *testing.T) {
	value := func(v uint64) *uint64 { return &v }
	withWindows := func(arm string, quiet, polling uint64, quietCalls, pollingCalls int) sustainedIORunArtifact {
		run := sustainedIOCheckpointRun(arm, map[string]uint64{"P2_small_edits": quiet + polling, "P5_main_advance": 1}, map[string]uint64{})
		run.Phases = append(run.Phases, sustainedIOPhaseReport{
			...
			Windows: []sustainedIOWindowReport{
				{Phase: "P8_idle_warm", Window: sustainedIOWindowIdleWarmPolling, LogicalWrites: value(polling), LogicalWritesExcl: value(polling), ClientCalls: pollingCalls},
				{Phase: "P8_idle_warm", Window: sustainedIOWindowIdleWarmQuiet, LogicalWrites: value(quiet), LogicalWritesExcl: value(quiet), ClientCalls: quietCalls},
			},
		})
		return run
	}
	set, err := sustainedIOFreezeBudgets([]sustainedIORunArtifact{withWindows("baseline", 100, 1_500_000, 0, 12)})
	if err != nil {
		t.Fatal(err)
	}
	budget := sustainedIOFindBudget(t, set, "P8_idle_warm")
	if len(budget.Windows) != 2 || budget.Windows[0].Window != sustainedIOWindowIdleWarmPolling {
	...
	verdict, err := sustainedIOCompareArms(set, []sustainedIORunArtifact{withWindows("candidate", 120, 1_400_000, 0, 12)})
	if err != nil {
		t.Fatal(err)
	}
	row := sustainedIOFindVerdict(t, verdict, "P8_idle_warm")
