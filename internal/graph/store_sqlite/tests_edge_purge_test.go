...
// TestOpenReplaysUnresolvedTestsPurgeFromFeatureV21 covers the historical
// feature lineage, which reached schema v21 before main's v14 purge merged.
// Such a store skips v14 by version, so the tip replay must still remove the
// stale derived edge without requiring a source reindex.
func TestOpenReplaysUnresolvedTestsPurgeFromFeatureV21(t *testing.T) {
	path := filepath.Join(t.TempDir(), "store.sqlite")

	s, err := Open(path)
	require.NoError(t, err)
	s.AddNode(&graph.Node{
		ID: "r/a_test.go::TestFeatureLineage", Kind: graph.KindFunction,
		Name: "TestFeatureLineage", FilePath: "r/a_test.go", RepoPrefix: "r",
	})
	s.AddEdge(&graph.Edge{
		From: "r/a_test.go::TestFeatureLineage", To: graph.UnresolvedMarker + "Gone",
		Kind: graph.EdgeTests, FilePath: "r/a_test.go", Line: 9,
	})
	require.NoError(t, s.Close())

	withRawDB(t, path, func(db *sql.DB) {
		_, err := db.Exec(`PRAGMA user_version = 21`)
		require.NoError(t, err, "reset to the pre-merge feature version")
	})

	s2, err := Open(path)
	require.NoError(t, err)
	t.Cleanup(func() { _ = s2.Close() })
	require.Empty(t, s2.GetOutEdges("r/a_test.go::TestFeatureLineage"),
		"the tip replay must purge a feature-v21 store's unresolved tests edge")
	version, err := readUserVersion(s2.writerDB)
	require.NoError(t, err)
