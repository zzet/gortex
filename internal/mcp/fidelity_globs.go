...
const fidelityGlobsParamDescription = "Ordered, comma-separated `glob:fidelity` rules used with compress_bodies; fidelity is full | compress | omit (e.g. \"internal/**:full,*_test.go:omit,vendor/**:compress\"). The first repo-relative match wins; unmatched files use compress. `*` stays within one segment; basename patterns match at any depth; trailing `/**`, a bare directory prefix, and trailing `/*` include its subtree; leading `**/` matches at any depth. Oversize input or more than 64 rules rejects the request. Per-symbol `keep` overrides compress or omit."
