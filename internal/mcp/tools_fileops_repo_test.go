...
func TestAnchorUnprefixedExisting(t *testing.T) {
	...
	abs, rel, n, err := anchorUnprefixedExisting(mi, viewPathRoot{}, "internal/x.go")
	...
	_, _, n, err = anchorUnprefixedExisting(mi, viewPathRoot{}, "shared/y.go")
	require.NoError(t, err)
	require.Equal(t, 2, n)

	// Missing everywhere (a new write target) → no anchor.
	_, _, n, err = anchorUnprefixedExisting(mi, viewPathRoot{}, "internal/new.go")
	require.NoError(t, err)
	require.Equal(t, 0, n)

	// A `..` traversal is rejected by the containment gate, not anchored.
	_, _, n, err = anchorUnprefixedExisting(mi, viewPathRoot{}, "../escape.go")
	require.NoError(t, err)
	require.Equal(t, 0, n)

	// An absolute path is never an "unprefixed" path — left to the caller.
	_, _, n, err = anchorUnprefixedExisting(mi, viewPathRoot{}, filepath.Join(a, "internal", "x.go"))
	require.NoError(t, err)
	require.Equal(t, 0, n)
}
