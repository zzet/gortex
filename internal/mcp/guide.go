...
- **MCP 2026 Streamable HTTP** at ` + "`POST /mcp`" + ` — mounted by ` + "`gortex daemon --http-addr <addr>`" + `, which serves ` + "`/mcp`" + ` and the ` + "`/v1`" + ` REST surface on one listener (non-localhost binds require ` + "`--http-auth-token`" + `).
...
Every response carries freshness metadata. ` + "`exact:false`" + ` means the requested view was not served, and every fallback is read-only. Set ` + "`require_exact:true`" + ` to reject fallback, ` + "`require_fresh:true`" + ` to wait for current filesystem state, and bound waiting with an absolute RFC3339 ` + "`wait_deadline`" + ` (a malformed or past deadline is refused).

` + "`require_fresh`" + ` waits for the selected checkout's route to reach the working copy as the coordinator samples it now, and reports what the wait did: ` + "`fresh`" + `, ` + "`waited_ms`" + `, ` + "`wait_deadline`" + `, and ` + "`fresh_reason`" + ` when it is false. ` + "`fresh:true`" + ` is reported only when the answer is read through the very route the wait published — the same checkout, served exactly, at the same route epoch; anything else answers ` + "`fresh:false`" + `. The ` + "`fresh_reason`" + ` vocabulary is ` + "`deadline_exceeded`" + ` (your bound expired with the route still behind the working copy), ` + "`interrupted`" + ` (the request ended first), ` + "`refresh_admission_abandoned`" + ` (the coordinator gave up on a bound of its own before admitting any refresh, so a longer wait_deadline is not the fix), ` + "`coordinator_unavailable`" + ` (this server can publish no checkout generation at all), ` + "`checkout_root_changed`" + ` and ` + "`checkout_refresh_stopped`" + ` (this one checkout moved, or its coordinator stopped), ` + "`publication_failed`" + `, ` + "`route_withdrawn`" + ` (publication succeeded and something else answered: a base fallback, another checkout, or a route that moved again), ` + "`committed_base_advance_unimplemented`" + `, and ` + "`wait_target_unavailable`" + ` (the route could not be resolved to wait on, or read back afterwards — a retry may succeed).

Without ` + "`require_exact`" + `, every ` + "`fresh:false`" + ` outcome still answers, out of the read-only stale route, with the reason. With ` + "`require_exact:true`" + ` set as well, every ` + "`fresh:false`" + ` outcome refuses instead: the pair means "a route that reflects the working copy, or nothing". A view that reads a committed base (the shared corpus, a labelled ` + "`base`" + ` graph, a ref or commit view, or a dedicated/primary checkout) has no working-copy route to advance, so it reports ` + "`committed_base_advance_unimplemented`" + ` without waiting — and refuses under ` + "`require_exact`" + ` like any other unfresh outcome.

All three are request-level arguments of the call, not parameters of any one tool: send them at the top level (or inside a facade ` + "`options`" + ` envelope) on any tool. They are accepted everywhere and published in no tool's input schema — three more properties on every tool would grow every ` + "`tools/list`" + ` — so this guide is where the contract lives.
