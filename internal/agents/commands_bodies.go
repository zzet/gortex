// Command, skill, and sub-agent body constants. Every adapter reads them
// from here and wraps them in its own frontmatter dialect.
//
// Adapters import this package; this package imports no adapter.
...
// SlashCommands maps a command's file name to its markdown content.
// Each entry is one slash command; the ".md" suffix in the key is the
// on-disk file name a host that discovers commands by file name uses.
...
// GlobalSkills maps the skill ID to its full SKILL.md body with YAML
// frontmatter. Each entry carries `name` + `description` so a host can
// list it in a skill picker. An adapter whose host wants a different
// frontmatter dialect strips this one with skillpack and puts its own
// envelope back on.
...
// SubAgents maps a sub-agent's file name to a graph-only definition.
// Each allowlist names tools present on the compact MCP surface received
// by every named client, and is the boundary that keeps a delegate off
// Bash/Grep/Glob — the body's prose is not.
...
// ---------------------------------------------------------------------------
// Slash-command / skill body constants
// ---------------------------------------------------------------------------
