...
- **MCP 2026 Streamable HTTP** at `POST /mcp` — mounted by `gortex daemon --http-addr <addr>`, which serves `/mcp` and the `/v1` REST surface on one listener (non-localhost binds require `--http-auth-token`). There is no top-level `gortex server` command — `gortex daemon server` edits the `servers.toml` roster — and `internal/server.Handler` routes `/mcp` only once `SetStreamableTransport` has been called, which nothing in production does.
