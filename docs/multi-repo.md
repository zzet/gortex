...
- `GORTEX_SQLITE_STABLE_READ_POOL=1` — retain each bounded SQLite read connection after it is opened instead of shedding idle connections. This is a recovery mode for hosts where connection churn makes WAL-index shared-memory mappings fault or otherwise prove unreliable; it preserves nested-read concurrency and the on-disk graph at the cost of retaining up to four reader page caches and mappings.
